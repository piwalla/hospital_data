...
- [x] **재활기관 데이터 추가** ✅ 완료 (2025-01-14)
  - [x] 재활기관 API 확인 및 테스트
    - [x] 공공데이터포털 API 확인 (근로복지공단\_산재재활기관관리정보)
...
### Phase 1.4: 산재 승인/요양 통계 개인화 (신규) 🔴

- [x] **데이터베이스 스키마 설정** (Priority 1) 🔴
  - [x] `industrial_accident_stats` 테이블 생성
    - [x] 컬럼: `id`, `injury_name`, `region_name`, `total_applications`, `total_approvals`, `avg_processing_days`, `avg_treatment_days`
- [x] **데이터 수집 및 통계 산출** (Priority 1) 🔴
  - [x] API 데이터 수집 스크립트 작성 (`scripts/fetch-accident-stats.ts`)
  - [x] 배치 처리 및 DB 저장 로직 구현
- [x] **대시보드 UI 구현** (Priority 1) 🔴
  - [x] `PersonalizedStatsWidget` 컴포넌트 생성
  - [x] API Route 생성 (`app/api/stats/personalized/route.ts`)
  - [x] 대시보드 연동
