...
사용자가 위치 기반으로 산재 지정 병원과 약국을 찾아 지도에서 확인하고 상세 정보를 조회
...
- **지도 표시**
  ...
  - 사용자 위치 기준 반경 5km 내 병원/약국 표시
  - 핀 형태로 시각화

- **상세 정보**

  - 핀/리스트 클릭 시 병원명, 주소, 연락처, 진료과목 표시
  - Bottom Sheet 형태로 표시
  - 전화 걸기, 길찾기 기능 포함
...
| 구성 요소 | 기술 | 목적 |
|----------|------|------|
| **Database** | Supabase (PostgreSQL) | 병원/약국 데이터 캐시, 사용자 로그 저장 |
...
| API | 엔드포인트 | 용도 | 요청 빈도 |
|-----|-----------|------|----------|
| **공공데이터포털** | `data.go.kr` | 산재 지정 의료기관 정보 조회 | 실시간 |
...
1. **진입 및 탐색**: 박OO씨가 위치 권한을 허용하자 주변 산재 지정 병원이 지도에 표시
2. **정보 확인**: 정형외과 필터 적용 후 가까운 병원 선택, 상세 정보 확인 및 전화 연락
