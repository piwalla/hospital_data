...
export default function ActionChecklist({ actions, initialCompletedIds }: ActionChecklistProps) {
  ...
  const t = dashboardTranslations[locale]?.checklist || dashboardTranslations['ko'].checklist;
  ...
  return (
    <div className="border-white/40 bg-white/80 backdrop-blur-md shadow-premium rounded-none border-x-0 sm:border sm:rounded-3xl flex flex-col h-full overflow-hidden transition-all hover:shadow-premium-hover relative group">
      <div className="p-6 sm:p-8 border-b border-primary/10 bg-white/40 backdrop-blur-sm flex justify-between items-center relative z-10">
        <h3 className="font-exrabold text-xl text-slate-900 flex items-center gap-3 tracking-tight">
          <div className="w-10 h-10 rounded-2xl bg-secondary flex items-center justify-center shadow-inner">
             <div className="w-3 h-3 bg-primary rounded-full animate-pulse shadow-[0_0_8px_rgba(20,83,45,0.5)]" />
          </div>
          {t.title}
        </h3>
        {pendingCount > 0 ? (
          <span className="px-3 py-1.5 rounded-full bg-slate-100/80 text-slate-600 text-sm font-black shadow-sm border border-slate-200 backdrop-blur-md">
            {t.remaining.replace('{count}', pendingCount.toString())}
          </span>
        ) : (
          <span className="px-3 py-1.5 rounded-full bg-secondary/80 text-primary text-sm font-black shadow-sm border border-secondary backdrop-blur-md flex items-center gap-1">
            <Check className="w-3 h-3" strokeWidth={4} /> {t.complete}
          </span>
        )}
      </div>
      ...
      <div className="p-4 sm:p-6 overflow-y-auto flex-1 custom-scrollbar">
        {actions.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center space-y-4">
            ...
            <p className="font-bold text-slate-500 whitespace-pre-wrap">{t.empty}</p>
          </div>
        ) : (
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isChecked 
                        ...
                        : "bg-white hover:bg-secondary/30 border-white hover:border-primary/20 shadow-sm hover:shadow-md"
                    )}
                  >
                    <div className={cn(
                      ...
                      isChecked
                        ...
                        : "border-slate-200 bg-white group-hover:border-primary group-hover:shadow-[0_0_0_3px_rgba(20,83,45,0.1)]"
                    )}>
                    ...
                    <div className="relative z-10 w-full">
                      <span className={cn(
                        ...
                        isChecked ? "text-slate-400 line-through decoration-2 decoration-slate-200" : "text-slate-800 group-hover:text-primary"
                      )}>
                        {action.title}
                      </span>
                      {action.description && (
                    ...
                    {!isChecked && (
                       <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                          <div className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(20,83,45,0.8)]" />
                       </div>
                    )}
                  </button>
                </li>
